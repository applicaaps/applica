...
function getGoogleCalendarLink(event: ProgramEvent) {
...
function downloadAppleIcs(event: ProgramEvent) {
...
// Lista eventi da popolare con gli eventi ufficiali dell'associazione
const eventsList: ProgramEvent[] = []
          ...
          <div className="space-y-5 sm:space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <span className="text-xs sm:text-sm text-[var(--color-on-surface-variant)] bg-[var(--color-surface-container)] px-2.5 py-1 rounded-md font-medium">
                {eventsList.length} {eventsList.length === 1 ? "incontro" : "incontri"} in elenco
              </span>
            </div>

            {eventsList.length === 0 ? (
              <RevealSection>
                <div className="bg-white rounded-2xl border border-[var(--color-outline-variant)] p-8 sm:p-12 text-center max-w-2xl mx-auto space-y-4 my-6 shadow-sm">
                  <div className="w-14 h-14 bg-[var(--color-primary)]/10 text-[var(--color-primary)] rounded-full flex items-center justify-center mx-auto">
                    <CalendarX size={28} />
                  </div>
                  ...
                  <p className="text-sm text-[var(--color-on-surface-variant)] leading-relaxed">
                    Stiamo aggiornando il calendario ufficiale degli incontri per la nuova stagione. Il programma dettagliato verrà pubblicato a breve!
                  </p>
                </div>
              </RevealSection>
            ) : (
